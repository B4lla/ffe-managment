        ...
        <div class="max-w-3xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg p-6">
                ...
                @if($provisional)
                    ...
                    <form method="POST" action="{{ route('convenios.firmar_empresa.store', $convenio->id) }}" enctype="multipart/form-data" class="space-y-4 border-t pt-5">
                        @csrf
                        <div>
                        ...
                        <div class="flex justify-end">
                            <button type="submit" class="inline-flex items-center px-4 py-2 bg-indigo-600 text-white rounded-md hover:bg-indigo-700">Subir firmado</button>
                        </div>
                    </form>

                    <form method="POST" action="{{ route('convenios.reportar_error_empresa', $convenio->id) }}" class="space-y-4 border-t pt-5 mt-5">
                        @csrf
